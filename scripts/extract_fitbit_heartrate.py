...
def extract_data(**kwargs):
    ...
    activity_request = requests.get('https://api.fitbit.com/1/user/' + API_ID + '/activities/heart/date/today/1d/1sec.json',
                                    headers={'Authorization': 'Bearer ' + ACCESS_TOKEN})
    
    # Debug
    print('Status Code: ', activity_request.status_code)

    oneDayData = activity_request.json()

    # Call data dump function to write out to CSV in specified directory
    filename = "./dags/files/files_heartrate/heartrate_data_" + data_hour + ".json"
    dump_data(oneDayData, filename)
    ...
    task_instance.xcom_push(key='api_result_heatrate', value=process_file)